...
// untuk pendaftaran
	public function index() {
        ...
        $data['error'] = null;
		$this->load->view('public/v_daftar',$data);
	}
	...
	public function proses_tambah_data_pendaftar()
	{
		...
		if ($this->form_validation->run() == FALSE)
        {
			...
			$this->load->view('public/v_daftar',$a);
        }else{
	...
	public function HitungY(){
		$kriteria = $this->M_kriteria->get_data_id_kriteria(); 
		$namaKolom = ['NMT_INVESTASI','NMT_SDM','NMT_LEGALITAS_USAHA','NMT_IZIN_INDUSTRI','NMT_SERTIFIKAT_SNI','NMT_PARTISIPASI','NMT_KETERSEDIAAN_MESIN','NMT_KONDISI_MESIN','NMT_TAHUN_PEMBUATAN_MESIN'];
		$data = null;
		for ($i=0; $i < sizeof($namaKolom); $i++) {
			...
			if($kriteria[$i]->TIPE_KRITERIA=='benefit'){
				$data['Y_MIN'] = 	$this->M_normalisasi->get_min_by_kolom($namaKolom[$i])->$namaKolom[$i];
				$data['Y_MAX'] = 	$this->M_normalisasi->get_max_by_kolom($namaKolom[$i])->$namaKolom[$i];
			}else if($kriteria[$i]->TIPE_KRITERIA=='cost'){
				$data['Y_MAX'] = 	$this->M_normalisasi->get_min_by_kolom($namaKolom[$i])->$namaKolom[$i];
				$data['Y_MIN'] = 	$this->M_normalisasi->get_max_by_kolom($namaKolom[$i])->$namaKolom[$i];
			}
	...
	public function edit_data_pendaftar($ID_PENDAFTAR)
	{
		...
		$this->load->view('public/pendaftar/v_edit_pendaftar', $data);
	}
	...
	{	
		...
		redirect('public/C_pendaftaran/tampil_view');
	}
		...
		public function detail_data_pendaftar($ID_PENDAFTAR)
	{
		...
		$this->load->view('public/pendaftar/v_form_detail', $data);
	}
	...
	{
		$data['daftar_pendaftar'] = $this->M_pendaftaran->tampil_data($this->session->userdata('USER_NAME'));
		$this->load->view('public/pendaftar/v_pendaftar',$data);
	}
